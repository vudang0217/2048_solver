' This code sets up the game board and can be controlled using '
...
class Game:
        ...
        while True:
            if auto:
                if self.checkIfCanGo():
                    ...
                    # USING THE AI, Gametree is in AI.
                    ai = Gametree(copy.deepcopy(self.tileMatrix),
                                  3, self.total_points)
        ...
        for i in range(0, self.board_size):
            for j in range(0, self.board_size):
                ...
                label = self.myfont.render(
                    str(self.tileMatrix[i][j]), 1, (255, 255, 255))
                label2 = self.scorefont.render(
                    "Score:" + str(self.total_points), 1, (255, 255, 255))
                self.surface.blit(
                    label, (i*(400/self.board_size) + 30, j*(400/self.board_size) + 130))
    ...
    def loadGameState(self):
        ...
        for i in range(0, self.board_size ** 2):
            self.tileMatrix[int(i / self.board_size)][i %
                                                      self.board_size] = int(m[i])
        f.close()
        self.loop(True)
